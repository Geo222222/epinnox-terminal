(() => {
  ...
  async function refreshPresets() {
    const select = document.getElementById('namedPreset');
    if (!select) return;
    try {
      const body = await fetch('/api/settings/presets').then(r => r.json());
      presetCache = Array.isArray(body.presets) ? body.presets : [];
      const selected = select.value;
      select.innerHTML = '<option value="">Saved preset…</option>' + presetCache.map(p => `<option value="${escapeHtml(p.name)}">${escapeHtml(p.name)}</option>`).join('');
      if (presetCache.some(p => p.name === selected)) select.value = selected;
    } catch (error) {
      setPresetStatus(`Preset load failed · ${error.message}`);
    }
  }
  ...
  async function saveNamedPreset() {
    ...
    if (!name) return setPresetStatus('Enter a preset name first.');
    try {
      const response = await fetch('/api/settings/presets', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name, schema_version: SCHEMA_VERSION, payload: collectWorkspace().values }),
      });
      const body = await response.json();
      if (!response.ok) throw new Error(body.detail || 'Preset save failed');
      await refreshPresets();
      document.getElementById('namedPreset').value = name;
      setPresetStatus(`Saved “${name}”.`);
    } catch (error) {
      setPresetStatus(`Preset save failed · ${error.message}`);
    }
  }

  function applySelectedPreset() {
    const name = document.getElementById('namedPreset')?.value;
    const preset = presetCache.find(p => p.name === name);
    if (!preset) return setPresetStatus('Choose a saved preset first.');
    for (const [id, value] of Object.entries(preset.payload || {})) setField(id, value, true);
    const tf = preset.payload?.timeframe;
    const tfButton = tf && document.querySelector(`.tf[data-tf="${tf}"]`);
    if (tfButton) tfButton.click();
    saveWorkspace();
    setPresetStatus(`Applied “${name}”.`);
  }

  async function deleteSelectedPreset() {
    const name = document.getElementById('namedPreset')?.value;
    if (!name) return setPresetStatus('Choose a saved preset first.');
    try {
      const response = await fetch(`/api/settings/presets/${encodeURIComponent(name)}`, { method: 'DELETE' });
      const body = await response.json();
      if (!response.ok) throw new Error(body.detail || 'Preset delete failed');
      await refreshPresets();
      setPresetStatus(`Deleted “${name}”.`);
    } catch (error) {
      setPresetStatus(`Preset delete failed · ${error.message}`);
    }
  }

  async function recoverPaper() {
    const button = document.getElementById('paperRecover');
    ...
    try {
      ...
      await refreshPaperRecoveryState();
    }
  }

  async function refreshPaperRecoveryState() {
    const recover = document.getElementById('paperRecover');
    const badge = document.getElementById('paperBadge');
    if (!recover || !badge) return;
    try {
      const state = await fetch('/api/paper-live/status').then(r => r.json());
      const needsRecovery = state.status === 'RECOVERY_REQUIRED';
      recover.classList.toggle('hidden', !needsRecovery);
      if (state.status && !['RUNNING','STOPPED'].includes(state.status)) {
        badge.textContent = state.status.replaceAll('_', ' ');
        badge.className = `status-pill ${state.status === 'RECOVERED' ? 'long' : 'neutral'}`;
      }
    } catch { /* app poll remains primary status surface */ }
  }
  ...
  async function bootPersistence() {
    ...
    if (!ready) return;
    let config = null;
    try { config = await fetch('/api/config').then(r => r.ok ? r.json() : null); } catch { /* app defaults remain */ }
    ...
    document.getElementById('presetSave')?.addEventListener('click', saveNamedPreset);
    ...
    document.getElementById('namedPreset')?.addEventListener('change', e => { if (e.target.value) document.getElementById('presetName').value = e.target.value; });
    document.getElementById('paperRecover')?.addEventListener('click', recoverPaper);
    window.addEventListener('beforeunload', () => { saveWorkspace(); saveSessionUi(); });
    await refreshPresets();
    await refreshPaperRecoveryState();
    setInterval(refreshPaperRecoveryState, 2500);
  }
