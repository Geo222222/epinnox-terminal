...
let chart,indicator,candleSeries,volumeSeries,currentMode='BACKTEST',paperPoll=null,autoTimer=null,backtestController=null,requestGeneration=0,booting=true;
const palette=['#60a5fa','#f59e0b','#22c55e','#f87171','#a78bfa','#22d3ee','#fb923c','#86efac'];
const strategyControlIds=['strategy','confirm1','confirm2','confirmationPolicy','confirmationRequired','confirmationWindow','direction','bars','balance','leverage','allocation','pyramiding','entryFee','exitFee','extraCost','netTarget','referral','maintenance','maxBars','stopLoss','manualParams'];
...
async function loadConfig(){
  ...
  $('strategy').value='Supertrend';$('confirmationPolicy').value='Single';
  ...
  syncConfirmationControls();updateStaticHeader();updateAccountGlance();
}
...
function draw(data){
  ...
  const p=data.open_position;if(p){candleSeries.createPriceLine({price:p.avg_entry,color:'#60a5fa',lineWidth:1,title:'AVG',axisLabelVisible:true});candleSeries.createPriceLine({price:p.break_even,color:'#f59e0b',lineWidth:1,lineStyle:2,title:'B/E'});candleSeries.createPriceLine({price:p.profit_target,color:'#22c55e',lineWidth:1,title:'TARGET'});candleSeries.createPriceLine({price:p.liquidation,color:'#ef4444',lineWidth:1,lineStyle:2,title:'LIQ ~'});}
  chart.timeScale().fitContent();render(data);
}

function render(d){
  const m=d.metrics;const cls=v=>Number(v)>=0?'positive':'negative';const model=d.entry_model||{label:d.strategy,policy:'Single',window_bars:1};$('entryModelSummary').textContent=`${model.label} · ${model.policy}`;
  const metricDefs=[['Net P&L',m.total_equity_pnl,4],['Open P&L',m.open_pnl,4],['Profit factor',m.profit_factor,3],['Win rate',m.win_rate_pct,2,'%'],['Max drawdown',m.max_drawdown_pct,2,'%'],['Target hit',m.target_hit_rate_pct,2,'%'],['Median hold',m.median_bars_to_exit,1,' bars'],['Fees paid',m.fees_paid,4],['Referral',m.referral_revenue,4],['Trades/day',m.trades_per_day,2],['Revenue/day',m.referral_revenue_per_day,4],['Closed trades',m.closed_trades,0]];
  $('tester').innerHTML=`<div class="metrics">${metricDefs.map(([a,b,digits,suffix=''])=>`<div class="metric"><span class="muted">${a}</span><b class="${a.includes('P&L')?cls(b):''}">${n(b,digits)}${suffix}</b></div>`).join('')}</div><p class="muted"><b style="color:#aebdce">Entry model:</b> ${esc(model.label)} · ${esc(model.policy)} · ${model.window_bars} bar confirmation window</p>`;
  $('tradeRows').innerHTML=d.trades.slice().reverse().map(t=>`<tr><td>${t.trade}</td><td class="${t.side==='long'?'positive':'negative'}">${esc(t.side.toUpperCase())}</td><td title="${esc(receiptSummary(t.entry_confirmation))}">${esc(receiptSummary(t.entry_confirmation))}</td><td>${t.layers}</td><td>${n(t.entry_price)}</td><td>${n(t.exit_price)}</td><td class="${cls(t.net_pnl)}">${n(t.net_pnl,4)}</td><td>${n(t.entry_fee,4)}</td><td>${n(t.exit_fee,4)}</td><td>${n(t.total_fee,4)}</td><td>${n(t.referral_commission,4)}</td><td>${n(t.leverage,0)}x</td><td>${n(t.margin,2)}</td><td>${n(t.mae_pct,3)}%</td><td>${n(t.mfe_pct,3)}%</td><td>${t.bars_held}</td><td>${esc(t.exit_reason)}</td></tr>`).join('');
...
function scheduleBacktest(delay=320){
  updateStaticHeader();updateAccountGlance();if(booting||currentMode!=='BACKTEST')return;clearTimeout(autoTimer);setStatus('Strategy settings changed · recalculation queued…');autoTimer=setTimeout(()=>runBacktest(),delay);
}
async function runBacktest(){
  ...
  try{const r=await fetch('/api/backtest',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload),signal:backtestController.signal});const d=await r.json();if(!r.ok)throw new Error(d.detail||'Backtest failed');if(generation!==requestGeneration)return;draw(d);setStatus(`Applied · ${d.entry_model.label} · ${d.metrics.closed_trades} closed trades`)}catch(e){if(e.name!=='AbortError')setStatus(`Backtest error · ${e.message}`)}finally{if(generation===requestGeneration){showRecalc(false);$('autoApplyBadge').textContent='AUTO APPLY'}}
}
...
for(const id of strategyControlIds){const el=$(id);if(!el)continue;if(id==='confirmationPolicy')continue;const evt=(el.tagName==='SELECT')?'change':'input';el.addEventListener(evt,()=>{if(id==='strategy'||id==='confirm1'||id==='confirm2')updateStaticHeader();scheduleBacktest(evt==='input'?420:100)})}
