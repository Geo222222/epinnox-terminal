(()=>{
  ...
  let refreshTimer=null,clockTimer=null,universeObserver=null,universeObservedEl=null,bodyObserver=null;
  let universeSparkToken=0,refreshInFlight=false,decorateScheduled=false,lastUniverseSymbol=null;
  ...
  function installFooter(){
    ...
    bar.appendChild(foot);bar.appendChild(existing);tickClock();clockTimer=setInterval(tickClock,1000);
  }
  ...
  function ensureUniverseDecor(){
    ...
    if(line&&!line.querySelector('.universe-token-icon')){
      ...
      const fav=document.createElement('button');fav.type='button';fav.id='universeFavorite';fav.className='universe-favorite';fav.title='Favorite symbol';fav.textContent='☆';line.appendChild(fav);fav.addEventListener('click',toggleUniverseFavorite);changed=true;
    }
    if(!$('universeMarketCard')){
      ...
      const body=$('universeInspectorBody');if(body){body.insertAdjacentElement('afterend',card);changed=true}
    }
    if(attachUniverseSymbolObserver())changed=true;
    if(changed)updateUniverseDecoration(true);
  }

  function attachUniverseSymbolObserver(){
    const el=$('universeSymbol');if(!el)return false;
    if(universeObserver&&universeObservedEl===el)return false;
    universeObserver?.disconnect();universeObservedEl=el;
    universeObserver=new MutationObserver(()=>updateUniverseDecoration());
    universeObserver.observe(el,{childList:true,characterData:true,subtree:true});return true;
  }

  function currentUniverseSymbol(){const t=$('universeSymbol')?.textContent?.trim()||'';return t.includes('/')?t:''}
  function updateUniverseDecoration(force=false){
    const symbol=currentUniverseSymbol(),holder=document.querySelector('.universe-token-icon'),changed=force||symbol!==lastUniverseSymbol;
    if(holder&&changed){holder.dataset.symbol=symbol;setHtml(holder,symbol?iconMarkup(symbol,'lg'):'')}
    const fav=$('universeFavorite');if(fav){const set=loadFavorites();setText(fav,symbol&&set.has(symbol)?'★':'☆');fav.classList.toggle('active',!!symbol&&set.has(symbol));fav.disabled=!symbol}
    const card=$('universeMarketCard'),spark=$('universeMarketSpark');
    if(changed){
      lastUniverseSymbol=symbol;
      if(card)card.dataset.symbol=symbol;
      if(symbol)loadUniverseSpark(symbol);else setHtml(spark,'<span>Select a symbol to inspect recent price action.</span>');
    }
  }
  ...
  async function loadUniverseSpark(symbol){
    const token=++universeSparkToken;
    try{
      const data=await fetchMarket(symbol,100);if(token!==universeSparkToken||symbol!==currentUniverseSymbol())return;
      const closes=(data.candles||[]).map(c=>Number(c[4])).filter(Number.isFinite),first=closes[0],last=closes.at(-1),chg=first?((last/first)-1)*100:0;
      setHtml($('universeMarketSpark'),`${sparkline(closes.slice(-60),260,54)}<div><strong>${num(last,last>=100?2:4)}</strong><span>${chg>=0?'+':''}${num(chg,2)}%</span></div>`);
      const c=$('universeMarketChange');if(c){setText(c,`${chg>=0?'+':''}${num(chg,2)}%`);c.classList.toggle('negative',chg<0)}
    }catch{if(symbol===currentUniverseSymbol())setHtml($('universeMarketSpark'),'<span>Market sparkline unavailable.</span>')}
  }

  function tickClock(){setText($('chromeClock'),new Date().toLocaleTimeString([], {hour:'2-digit',minute:'2-digit',second:'2-digit'}))}
  function symbolOptions(){return [...($('symbol')?.options||[])].map(o=>({value:o.value,label:o.textContent||o.value})).filter(x=>x.value)}

  function renderSearchResults(){
    const q=($('chromeSymbolSearch')?.value||'').trim().toLowerCase(),box=$('chromeSearchResults');if(!box)return;
    if(!q){box.classList.add('hidden');box.innerHTML='';return}
    const rows=symbolOptions().filter(x=>x.value.toLowerCase().includes(q)||x.label.toLowerCase().includes(q)).slice(0,8);
    box.innerHTML=rows.length?rows.map(x=>`<button type="button" data-symbol="${esc(x.value)}">${iconMarkup(x.value,'xs')}<span><b>${esc(codeOf(x.value))}</b><em>${esc(x.value)}</em></span></button>`).join(''):'<span class="search-empty">No matching symbol</span>';
    box.classList.remove('hidden');box.querySelectorAll('[data-symbol]').forEach(b=>b.addEventListener('click',()=>{applySymbol(b.dataset.symbol);closePopovers()}));
  }
  ...
  async function showSystemPopover(kind){
    const box=$('chromeSystemPopover');if(!box)return;
    box.classList.toggle('hidden',box.dataset.kind===kind&&!box.classList.contains('hidden'));box.dataset.kind=kind;if(box.classList.contains('hidden'))return;
  ...
  function applySymbol(symbol){
    ...
    const option=[...select.options].find(o=>o.value===symbol)||[...select.options].find(o=>codeOf(o.value)===codeOf(symbol));if(!option)return;
    select.value=option.value;select.dispatchEvent(new Event('change',{bubbles:true}));if($('chromeSymbolSearch'))$('chromeSymbolSearch').value='';updateCurrentAsset();
  }

  async function fetchMarket(symbol,limit=100){
    const key=`${symbol}|1m|${limit}`,cached=responseCache.get(key);if(cached&&Date.now()-cached.at<12000)return cached.data;
    const controller=new AbortController(),timeout=setTimeout(()=>controller.abort(),8000);
    try{
      const r=await fetch(`/api/market?symbol=${encodeURIComponent(symbol)}&timeframe=1m&limit=${limit}`,{signal:controller.signal});const d=await r.json();if(!r.ok)throw new Error(d.detail||'market unavailable');responseCache.set(key,{at:Date.now(),data:d});return d;
    }finally{clearTimeout(timeout)}
  }

  async function refreshWatch(){
    if(document.hidden||refreshInFlight)return;refreshInFlight=true;
    try{
      await Promise.allSettled(WATCH.map(async item=>{
        const data=await fetchMarket(item.symbol,100),closes=(data.candles||[]).map(c=>Number(c[4])).filter(Number.isFinite);if(closes.length<2)return;
        const first=closes[0],last=closes.at(-1),chg=((last/first)-1)*100,card=document.querySelector(`[data-watch="${CSS.escape(item.symbol)}"]`);
        if(card){setText(card.querySelector('[data-price]'),num(last,last>=100?2:4));const ce=card.querySelector('[data-change]');setText(ce,`${chg>=0?'+':''}${num(chg,2)}%`);ce?.classList.toggle('negative',chg<0);setHtml(card.querySelector('[data-spark]'),sparkline(closes.slice(-34),52,15))}
        quoteCache.set(item.symbol,{at:Date.now(),price:last,change:chg,closes});
      }));
      renderFooterTickers();updateCurrentAsset();
    }finally{refreshInFlight=false}
  }

  function renderFooterTickers(){const el=$('chromeFooterTickers');if(!el)return;setHtml(el,WATCH.map(x=>{const d=quoteCache.get(x.symbol);return `<span><b>${x.code}</b><em class="${d?.change<0?'negative':''}">${Number.isFinite(d?.change)?`${d.change>=0?'+':''}${num(d.change,2)}%`:'—'}</em></span>`}).join(''))}

  function updateCurrentAsset(){
    const symbol=$('symbol')?.value||'';if(!symbol)return;const code=codeOf(symbol),icon=$('inspectorAssetIcon'),label=$('inspectorAssetSymbol'),strategy=$('strategy')?.value||'Strategy workspace';
    setHtml(icon,iconMarkup(symbol,'lg'));setText(label,symbol);setText($('inspectorAssetStrategy'),strategy);
    const badge=document.querySelector('.symbol-badge');if(badge){setHtml(badge,iconMarkup(symbol,'sm'));badge.title=code}
    const d=quoteCache.get(symbol)||quoteCache.get(WATCH.find(w=>w.code===code)?.symbol);if(d?.closes)setHtml($('inspectorAssetSpark'),sparkline(d.closes.slice(-32),76,24));
  }

  function scheduleDecorInstall(){
    if(decorateScheduled)return;decorateScheduled=true;
    requestAnimationFrame(()=>{decorateScheduled=false;installRailIcons();ensureUniverseDecor();installChartAssetHero()});
  }
  ...
  function observeDynamicUi(){
    ...
    $('symbol')?.addEventListener('change',()=>{updateCurrentAsset();scheduleDecorInstall()});
    $('strategy')?.addEventListener('change',updateCurrentAsset);
    document.addEventListener('epinnox:workbench-result',updateCurrentAsset);
  }
