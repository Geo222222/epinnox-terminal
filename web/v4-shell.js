(()=>{
  ...
  function ensureStyle(){if(document.querySelector('link[href="/static/product-chrome.css"]'))return;const l=document.createElement('link');l.rel='stylesheet';l.href='/static/product-chrome.css';document.head.appendChild(l)}
  function ensureCanonicalShell(){return new Promise(resolve=>{if(window.__EPINNOX_CANONICAL_SHELL__){resolve();return}let s=document.querySelector('script[src="/static/product-chrome.js"]');if(s){s.addEventListener('load',resolve,{once:true});setTimeout(resolve,250);return}s=document.createElement('script');s.src='/static/product-chrome.js';s.defer=true;s.addEventListener('load',resolve,{once:true});document.body.appendChild(s)})}
  ...
  function setChartActive(on){document.querySelectorAll('.ep-nav-item').forEach(a=>a.classList.toggle('active',on&&a.dataset.page==='Terminal'))}
  function activateSession(id){selected=id;sessionStorage.setItem('epinnox.v4.selectedSession',id);setChartActive(false);modeButton('PAPER')?.click();document.dispatchEvent(new CustomEvent('epinnox:session-selected',{detail:{sessionId:id}}))}
