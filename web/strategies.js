...
async function renderPresets(){try{const d=await fetch('/api/settings/presets').then(r=>r.json()),rows=Array.isArray(d.presets)?d.presets:[];$('strategyPresetCount').textContent=rows.length;$('strategyPresets').innerHTML=rows.length?rows.map(p=>{const v=p.payload||{};return `<div class="preset-row-card"><strong>${esc(p.name)}</strong><span>${esc(v.symbol||'Any symbol')} · ${esc(v.timeframe||'—')} · ${esc(v.strategy||'—')}</span></div>`}).join(''):'<div class="compact-note" style="padding:12px">No named presets yet.</div>'}catch(e){$('strategyPresetCount').textContent='—';$('strategyPresets').innerHTML=`<div class="compact-note" style="padding:12px">Preset load failed · ${esc(e.message)}</div>`}}
